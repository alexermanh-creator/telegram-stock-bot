...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif state and state.startswith('awaiting_edit_'):
        p = state.split("_"); tx_id, bd, amt = p[2], p[3], parse_amount(text)
        if amt is not None:
            conn = sqlite3.connect(DB_FILE); conn.execute("UPDATE transactions SET amount = ? WHERE id = ?", (amt, tx_id)); conn.commit(); conn.close()
            context.user_data.clear(); pg = None if bd=="recent" else int(bd); m, mk = get_history_menu(pg); await update.message.reply_text("✅ Đã sửa.\n\n" + m, reply_markup=mk)
...
async def handle_doc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.document.file_name == DB_FILE:
        f = await update.message.document.get_file(); await f.download_to_drive(DB_FILE); await update.message.reply_text("✅ Restore DB thành công!")
