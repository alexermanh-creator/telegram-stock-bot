...
def init_db():
    ...
    # Tự động bơm dữ liệu nếu DB trống
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Xem tài sản chi tiết (Theo yêu cầu của bạn)
    ...
    # Biểu đồ Pro (Area Chart)
        ...
        if txs:
            ...
            plt.plot(dates, caps, color='#1f77b4', linewidth=2, label='Vốn tích lũy', marker='o', markersize=3)
            plt.fill_between(dates, caps, color='#1f77b4', alpha=0.2)
            plt.gca().yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, p: f"{x/1000000:,.0f}M"))
            plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%m/%Y'))
            plt.grid(True, linestyle='--', alpha=0.5); plt.legend(); plt.title("BIẾN ĐỘNG VỐN")
        ...
        conn = sqlite3.connect(DB_FILE); rows = conn.execute("SELECT date, category, type, amount FROM transactions ORDER BY date DESC LIMIT 10").fetchall(); conn.close()
        msg = "📜 10 GIAO DỊCH GẦN NHẤT:\n" + "\n".join([f"• {r[0]}: {r[1]} {r[2]} {format_money(r[3])}" for r in rows])
        ...
        conn = sqlite3.connect(DB_FILE); rows = conn.execute("SELECT category, type, amount, date FROM transactions ORDER BY date DESC").fetchall(); conn.close()
        if rows:
...
async def handle_doc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.document.file_name == DB_FILE:
        ...
        await update.message.reply_text("✅ Restore dữ liệu thành công!", reply_markup=get_main_menu())
