...
# --- 4. LỊCH SỬ (GIỮ NGUYÊN FORM BẠN MUỐN) ---
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == '🏦 Quản lý Tài sản': await update.message.reply_text("🏦 QUẢN LÝ TÀI SẢN", reply_markup=get_asset_menu())
        ...
        msg = (f"🏆 *TỔNG TÀI SẢN*\n`{format_money(s['total_val'])}` VNĐ\n"
               ...
               f"🎯 Mục tiêu: {s['progress']:.1f}% (`{format_money(s['target_asset'])}`)\n"
               f"----------------------------------\n"
        ...
        if txs:
            ...
            plt.figure(figsize=(10, 6)); plt.plot(dates, caps, color='#1f77b4', linewidth=2, label='Vốn tích lũy', marker='o', markersize=3)
            plt.fill_between(dates, caps, color='#1f77b4', alpha=0.2); plt.gca().yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, p: f"{x/1000000:,.0f}M"))
            plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%m/%Y')); plt.grid(True, linestyle='--', alpha=0.5); plt.legend(); plt.title("BIẾN ĐỘNG VỐN")
    ...
    if state == 'awaiting_target':
        ...
        if 'hòa vốn' in text_l or 'hoà vốn' in text_l: nt = s['total_von']
            ...
            if m:
                d_val = 1 if m.group(1) in ['lãi', 'lời'] else -1; v, u = float(m.group(2)), m.group(3)
                if u == '%': nt = s['total_von'] + (s['total_von'] * (d_val * v / 100))
                else: nt = s['total_von'] + (d_val * (parse_amount(f"{v}{u or ''}") or 0))
        ...
        parts = state.split("_"); tx_id, back_to, amt = parts[2], parts[3], parse_amount(text)
        if amt is not None:
            conn = sqlite3.connect(DB_FILE); conn.execute("UPDATE transactions SET amount = ? WHERE id = ?", (amt, tx_id)); conn.commit(); conn.close()
            context.user_data.clear(); page = None if back_to == "recent" else int(back_to); msg, mk = get_history_menu(page)
            await update.message.reply_text("✅ Đã cập nhật số tiền.\n\n" + msg, reply_markup=mk)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("hist_"):
        ...
        page = None if p[2] == "recent" else int(p[2]); msg, mk = get_history_menu(page); await q.edit_message_text("✅ Đã xóa.\n\n" + msg, reply_markup=mk)
        ...
        context.user_data['state'] = f"awaiting_balance_{d.split('_')[1]}"; await q.edit_message_text(f"Nhập số dư hiện tại cho {d.split('_')[1]}:")
