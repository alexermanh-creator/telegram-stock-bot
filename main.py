...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ['/start', '🏠 Menu Chính']:
        context.user_data.clear(); await update.message.reply_text("🏠 DASHBOARD", reply_markup=get_main_menu()); return

    elif text in ['/xoa_tri_nho', '🧹 Xóa trí nhớ AI']:
        portfolio_ai.chat_history = []
        await update.message.reply_text("🧹 Đã làm trống bộ não AI."); return

    if text == '🏦 Quản lý Tài sản': await update.message.reply_text("🏦 QUẢN LÝ", reply_markup=get_asset_menu())
        ...
        await update.message.reply_text("🤖 AI đã sẵn sàng soi bảng tài sản. Gõ câu hỏi của bạn:", reply_markup=ReplyKeyboardMarkup([['🧹 Xóa trí nhớ AI', '🏠 Menu Chính']], resize_keyboard=True))
        return

    elif state == 'chatting_ai':
        s = get_stats(); d = s['details']
        loading = await update.message.reply_text("⌛ AI đang phân tích...")
        full_context = (f"NAV: {format_money(s['total_val'])}đ | Lãi: {s['total_lai_pct']:.1f}%\n"
                        f"Mục tiêu: {s['progress']:.1f}% đến {format_money(s['target_asset'])}\n"
                        f"Crypto: {format_money(d['Crypto']['hien_co'])} (Vốn {format_money(d['Crypto']['von'])})\n"
                        f"Stock: {format_money(d['Stock']['hien_co'])} (Vốn {format_money(d['Stock']['von'])})")
        ...
        msg = (f"🏆 *TỔNG TÀI SẢN*\n`{format_money(s['total_val'])}` VNĐ\n"
               ...
               f"🎯 Mục tiêu: {s['progress']:.1f}%\n"
               f"----------------------------------\n"
               f"🟡 *CRYPTO*: {format_money(d['Crypto']['hien_co'])}\n"
               f"📈 *STOCK*: {format_money(d['Stock']['hien_co'])}\n"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif d.startswith("cat_"): context.user_data.update({'state': f"awaiting_{d.split('_')[1]}", 'category': d.split('_')[2]}); await q.edit_message_text(f"Nhập tiền {d.split('_')[1]} cho {d.split('_')[2]}:")
